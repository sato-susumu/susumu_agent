...
このリポジトリでの ADK は「自然言語を解釈し、適切な Python 関数を Function Calling で呼び出す層」です。ロボットの実際の制御、安全状態、ROS2 との接続は ADK の外側にあり、ADK から見える操作面は `RobotTools` のメソッドだけです。
...
この情報から、ADK は `move_robot` という tool に `direction`、`speed`、`duration_sec` という引数があり、`direction` と `speed` は指定された候補から選ぶべきだと判断できます。ユーザーが「ゆっくり3秒前進」と入力した場合、LLM はこのスキーマとシステムプロンプトを使って、概念的には次のような tool call を作ります。
...
また、`Watchdog` が `SharedState.last_command_time` を監視しており、一定時間コマンド更新がない場合は停止方向に働きます。
