...
def _write_srt(entries: list[dict], path: Path) -> None:
    """entries: [{start_sec, end_sec, command, command_en, response}]"""
    lines = []
...
def _burn_subtitles(video_path: str, srt_path: str, output_path: str) -> bool:
    """ffmpeg で字幕を動画に焼き込む。成功したら True を返す。"""
    import subprocess
    cmd = [
        "ffmpeg", "-y",
        "-i", video_path,
...
def _make_gif(video_path: str, gif_path: str) -> bool:
    """ffmpeg で動画をアニメーション GIF に変換する（GitHub 向け小サイズ）。"""
    import subprocess
    # 320px 幅・8fps・256色パレット最適化
...
async def _run_demo(config: dict, node: Node, debug: bool = False, debug_dir: str = "debug") -> None:
    robot = ROS2Robot(node, "/turtle1/cmd_vel")
    tools_module.set_robot(robot)
    tools_module.set_camera(CameraClient(mode="simulate"))

    watchdog = Watchdog(timeout_sec=config.get("robot", {}).get("watchdog_timeout_sec", 5.0))
    watchdog.start()
    ...
    await session_service.create_session(
        app_name="robot_nl", user_id="demo", session_id=session_id
    )

    logger.info("turtlesim エージェントデモ開始")

    # デバッグモード時は turtlesim 録画を開始
    recorder = None
    ...
    try:
        for i, cmd in enumerate(DEMO_COMMANDS):
            logger.info(f"[{i+1}/{len(DEMO_COMMANDS)}] 入力: {cmd!r}")
            ...
            if debug:
                label_entries.append({
                    ...
                    "command": cmd,
                    "command_en": _COMMAND_EN.get(cmd, cmd),
                    "response": response,
                })
        ...
        if debug and label_entries:
            ts = datetime.datetime.fromtimestamp(
                record_start_time or datetime.datetime.now().timestamp()
            ).strftime("%Y%m%d_%H%M%S")
            label_path = Path(debug_dir) / f"{ts}_demo_labels.jsonl"
        ...
        # 字幕焼き込み → GIF 変換
        if debug and video_path and srt_entries and record_start_time:
...
def main() -> None:
    ...
    try:
        asyncio.run(_run_demo(config, node, debug=debug, debug_dir=debug_dir))
    finally:
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()
