# 自然言語ロボット制御システム 設計ドキュメント v6

> レビューサイクル33回を経て改訂。変更箇所は各セクションに ⚡ で示す。
...
```
robot_nl_controller/
├── main.py               # エントリポイント・入力ループ・緊急停止検出
├── agent.py              # LlmAgent定義・ツール実装（5ツール）
├── ros2_bridge.py        # rclpy ノード・SharedTwist・20Hz Timer
├── watchdog.py           # 無通信タイムアウト監視
├── camera.py             # Image Subscriber・base64変換・鮮度チェック
├── capabilities.py       # 速度定数・build_system_prompt()
├── requirements.txt      # pip インストール可能パッケージのみ
├── requirements-dev.txt  # pytest・mock 等
└── voice/
    ├── __init__.py
...
**requirements.txt（pip でインストール可能）：**

| パッケージ | バージョン | 用途 |
|---|---|---|
| `google-adk` | `>=2.1.0` | ADK本体 |
| `anthropic[vertex]` | `>=0.50.0` | Claude on Vertex AI |
| `google-cloud-aiplatform` | `>=1.90.0` | Vertex AI認証 |
| `opencv-python` | `>=4.9.0` | カメラ画像処理 |
...
| コマンド実行履歴 | `command_log.jsonl` | ローテーション（10MB×5） |
...
├── requirements-dev.txt      # pytest・mock
├── launch/
│   └── robot_nl.launch.py    # ROS2 launch ファイル
├── deploy/
...
```
robot_nl_controller/
├── config.yaml               # 全設定（トピック・モデル・モード・コスト制御等）
├── main.py                   # 入力ループ・緊急停止・コマンドキュー・フィードバック
├── agent.py                  # LlmAgent 定義のみ（8ツール登録）
├── tools.py                  # 全ツール実装（RobotInterface に依存）
├── shared_state.py           # SharedState 集約
├── robot/
│   ├── interface.py          # RobotInterface 抽象クラス
│   ├── ros2_robot.py         # ROS2 実装（QoS 設定含む）
│   └── mock_robot.py         # テスト・simulate 用
├── watchdog.py               # 無通信タイムアウト監視
├── camera.py                 # Image Subscriber・遅延起動・顔ブラー・鮮度チェック
├── capabilities.py           # 速度定数・バリデーション・プロンプト自動生成
├── session_store.py          # セッション・コマンド履歴 JSONL
├── macro_store.py            # マクロ登録・読み込み
├── metrics.py                # Prometheus メトリクス
├── requirements.txt          # バージョン上限付き
├── requirements.lock         # 完全固定版
├── requirements-dev.txt      # pytest・mock・pip-audit
├── launch/
│   └── robot_nl.launch.py
├── deploy/
│   ├── robot-nl.service
...
├── tests/
│   ├── unit/                 # Layer1: ROS2 不要
│   ├── mock/                 # Layer2: MockRobot 使用
│   └── golden/               # Layer3: 実 LLM・週1回
└── voice/
    ├── recognizer.py          # 抽象基底クラス（緊急キーワード優先検出含む）
    └── synthesizer.py         # 抽象基底クラス（テキストフォールバック実装）
```
