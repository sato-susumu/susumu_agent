"""MockRobot デバッグモード launch ファイル。

mock.launch.py の debug:=true 固定版。
ログを debug/ フォルダに保存する。

使い方:
    ros2 launch susumu_agent mock_debug.launch.py
    ros2 launch susumu_agent mock_debug.launch.py debug_dir:=/tmp/mydbg
    ros2 launch susumu_agent mock_debug.launch.py cmd_vel_stamped:=true
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        cmd_vel_stamped_arg,
        debug_dir_arg,
        LogInfo(msg="susumu_agent を起動します（MockRobot デバッグモード）..."),
        agent_node,
    ])
